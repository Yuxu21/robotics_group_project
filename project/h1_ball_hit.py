...
class TrajectoryNode(Node):
    ...
    def solve_ik(self):
        ...
        # ---------------------------------------------------------
        # 1) Orientation: axis-angle between R0 and R_goal
        # ---------------------------------------------------------
        R_err = R0.T @ R_goal
        tr = np.trace(R_err)
        ...
        if theta < 1e-6:
            # Almost no rotation: keep R = R0, no angular motion
            use_orientation = False
            k = np.array([1.0, 0.0, 0.0])
        else:
            use_orientation = True
            k = (1.0 / (2.0 * np.sin(theta))) * np.array([
                R_err[2, 1] - R_err[1, 2],
                R_err[0, 2] - R_err[2, 0],
                R_err[1, 0] - R_err[0, 1],
            ])
        ...
        for n in range(0, N + 1):
            ...
            # --- Desired orientation + angular velocity (scalar spline) ---
            if not use_orientation:
                # No rotation needed
                R_d = R0
                w_d = np.zeros(3)
            else:
                # alpha(t) goes from 0 to theta with alpha_dot(0)=alpha_dot(T)=0
                alpha, alpha_dot = spline(t, T, 0.0, theta, 0.0, 0.0)

                # Rotation about axis k by angle alpha
                R_d = R0 @ Rotn(k, alpha)

                # Angular velocity in base frame: w = R0 * (k * alpha_dot)
                w_d = R0 @ (k * alpha_dot)
