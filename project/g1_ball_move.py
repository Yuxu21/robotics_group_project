...
class TrajectoryNode(Node):
    ...
    def __init__(self, name, future):
        ...
        self.p_ball = np.array([0.1, 0.4, 0.2])
        x_ball = np.array([1.0,0.0,0.0])
        # the prehit position
        self.ppre=self.p_ball - 10*self.ball_radius * x_ball
        self.Rpre=Rotz(pi/2)
        ...
        self.T_hit   = self.hT+self.hT/2
        self.p0_ball = np.array([1.0, 1.0, self.ball_radius])
    ...
    def update(self):
        ...
        if   (t < self.hT):
            ...
            qd,qddot=goto(t,self.hT,self.q0,self.qpre)
        else:
            tt=t-self.hT
            s=cos(2*pi*tt/self.T)
            sdot=-2*pi/self.T*sin(2*pi*tt/self.T)
        ...
        # # )
        #     self.get_logger().info(f'ball is ={self.p}')

        #     self.get_logger().info(f'dist is={np.linalg.norm(self.p-pd)}')
